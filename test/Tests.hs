...
squeezed :: (0 <= x, x <= 0) => proxy x -> proxy 0
squeezed = id

squeezed2 :: (5 <= x, x <= 5) => Proxy x -> Proxy 5
squeezed2 = id

type family Rank (n :: Nat) :: Nat

bar :: forall a b. ((a <= b) ~ (() :: Constraint)) => Proxy (a :: Nat) -> Proxy b -> ()
bar _ _ = ()

foo :: forall perm sh1 sh2. (Rank perm <= Rank sh2, 1 + Rank perm ~ Rank sh1)
    => Proxy perm
    -> Proxy sh1
    -> Proxy sh2
    -> ()
foo _ _ _ = bar @(Rank sh1) @(1 + Rank sh2) Proxy Proxy

type family Max a b where
  Max 0 b = b

f :: Proxy a -> Proxy b -> Proxy d -> Proxy (a + (Max (a + b) d)) -> Proxy ((Max (b + a) d) + a)
f _ _ _ = id
...
tests = testGroup "ghc-typelits-sopsat"
  ...
  , testGroup "Inequality"
    ...
    , testCase "b ~ (2^a) => 1 <= b" $
      ...
      "Proxy"
    , testCase "a <= 0 => a ~ 0" $
      show (squeezed (Proxy :: Proxy 0)) @?=
      "Proxy"
    , testCase "a <= 5 && 5 <= a => a ~ 5" $
      show (squeezed2 (Proxy :: Proxy 5)) @?=
      "Proxy"
    ]
  , testGroup "errors"
    ...
    , testGroup "Inequality"
      ...
      , testCase "2 <= x && 2 >= x => x ~ 1" $
        squeezed3 (Proxy :: Proxy 2) `throws` squeezed3Errors
      , testCase "1 <= x && 1 >= x => x ~ 3" $
        squeezed4 (Proxy :: Proxy 1) `throws` squeezed4Errors
      , testCase "False :~: True" $
        urk `throws` urkErrors
      ]
    ]
  ]
...
test :: (KnownNat n, n <= 3) => Proxy n -> Integer
test = toInteger . natVal

foo1 :: Integer
foo1 = test (Proxy :: Proxy 2)

bar1 :: (KnownNat a, KnownNat (a - 4), 4 <= a, a <= 7) => Proxy (a - 4) -> Integer
bar1 = test

bar2 :: (KnownNat a, KnownNat b, KnownNat (b - 2), a ~ (b - 2), 2 <= b) => Proxy (a + 2) -> Proxy b
bar2 = id
